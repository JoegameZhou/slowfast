...
# ============================================================================
"""Help functions for processing AVA dataset."""
import logging
import os
...
def load_boxes_and_labels(cfg, mode):
    ...
    logger.info("Detection threshold: %s", detect_thresh)
...
def get_keyframe_data(boxes_and_labels):
    ...
    for video_idx in range(len(boxes_and_labels)):
        ...
        for sec in boxes_and_labels[video_idx].keys():
            ...
            if boxes_and_labels[video_idx][sec]:
...
def parse_bboxes_file(
    ...
    for video_name, _ in all_boxes.items():
