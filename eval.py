...
def run_eval():
    ...
    # load ckpt
    param_dict = load_checkpoint(cfg.TEST.CHECKPOINT_FILE_PATH, network)
    ...
    # start eval
    for cur_iter, (slowpath, fastpath, boxes, labels, ori_boxes, metadata, mask) in enumerate(dataset.create_tuple_iterator()):
        test_meter.data_toc()
        ...
        ori_boxes = np.concatenate((padded_idx, ori_boxes.asnumpy()), axis=2)[mask]
